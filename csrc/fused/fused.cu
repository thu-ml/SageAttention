...
template <uint32_t head_dim, uint32_t BLOCK_SIZE, uint32_t QUANT_BLOCK_SIZE, uint32_t num_pack_per_thread = 1, bool has_sm_scale = false, bool sub_mean = false, typename T>
...
#pragma unroll
  ...
  float *scale_ptr_base = scale + batch_id * stride_bz_scale + head_id * stride_h_scale + bx * (BLOCK_SIZE / QUANT_BLOCK_SIZE);

  constexpr uint32_t num_iters = BLOCK_SIZE / QUANT_BLOCK_SIZE;
...
#pragma unroll
  ...
  // loop from bottom to top so that we only need to initialize x_val_float once
  for (int i = num_iters - 1; i >= 0; i--)
  {
    // load the data
    if (thread_base_token + i * QUANT_BLOCK_SIZE < num_tokens)
    {
      *(float4*)(&x_val[0]) = *(float4*)(input_ptr_base + i * QUANT_BLOCK_SIZE * stride_seq_input);
#pragma unroll
      for (uint32_t j = 0; j < 8; j++)
      {
...
#pragma unroll
        for (uint32_t j = 0; j < 8; j++)
        {
          x_val_float[j] -= mean_val_float[j];
        }
      }
...
#pragma unroll
    ...
    if (thread_id == 0)
    {
      ...
      scale_ptr_base[i] = s_amax / 127.0f;
    }
...
#pragma unroll
    ...
    {
      o_val[j] = make_char4(
        float_to_int8_rn(x_val_float[j * 4 + 0] * tmp_scale),
        float_to_int8_rn(x_val_float[j * 4 + 1] * tmp_scale),
        float_to_int8_rn(x_val_float[j * 4 + 2] * tmp_scale),
        float_to_int8_rn(x_val_float[j * 4 + 3] * tmp_scale)
      );
    }
    ...
    if (thread_base_token + i * QUANT_BLOCK_SIZE < num_tokens)
    {
      *reinterpret_cast<float2*>(output_ptr_base + i * QUANT_BLOCK_SIZE * stride_seq_output) = *(float2*)(&o_val[0]);
    }
  }
}
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        constexpr int QUANT_BLOCK_SIZE = BLOCK_SIZE;
        ...
        dim3 block(BLOCK_SIZE * (HEAD_DIM / 8));

        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, QUANT_BLOCK_SIZE, 1, true, false, c_type><<<grid, block>>>(
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, QUANT_BLOCK_SIZE, 1, false, false, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
          nullptr,
          output.data_ptr<int8_t>(),
          reinterpret_cast<float*>(scale.data_ptr()),

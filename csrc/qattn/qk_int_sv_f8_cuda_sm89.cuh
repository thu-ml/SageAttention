...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypePVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_scale = false, bool fuse_v_mean=false>
                      ...
                      float sm_scale)
{
  ...
  static_assert(std::is_same<DTypePVAccum, float>::value, "DTypePVAccum must be float, half is WIP");
  ...
  cudaStream_t stream)
{
  ...
  auto kernel_func = qk_int_sv_f8_attn_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, head_dim, DataType::kInt8, static_cast<QuantGranularity>(qk_quant_gran), static_cast<QuantGranularity>(qk_quant_gran), DTypePVAccum, use_inst_buffer, DTypeOut, ComputeUnit::kCudaCore, mask_mode, return_lse, fuse_v_scale, fuse_v_mean>;

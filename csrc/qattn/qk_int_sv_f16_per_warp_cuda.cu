...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK,
        typename DTypeSVAccum = float, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_mean=false>
__global__ void qk_int_sv_f16_attn_per_warp_kernel(int8_t *__restrict__ Q, int8_t *__restrict__ K, half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      ...
                      float sm_scale)
{
  ...
  const uint32_t num_block_k = div_ceil(kv_len, CTA_K) * (CTA_K / WARP_K);

  uint32_t q_scale_idx = batch_id * num_qo_heads * num_warp_block_q + head_id * num_warp_block_q + bx * num_warps_q + get_warp_idx_q<num_warps_q, num_warps_k>();
  uint32_t k_scale_idx = batch_id * (num_qo_heads / num_kv_groups) * num_block_k + (head_id / num_kv_groups) * num_block_k;
...
#pragma unroll
  ...
  float dequant_scale = q_scale * K_scale[k_scale_idx + 0];
...
#pragma unroll
    ...
    dequant_scale = q_scale * K_scale[k_scale_idx + num_iterations - 1];
    sm_scale = original_sm_scale * dequant_scale;
    ...
    DISPATCH_CAUSAL(is_causal, IS_CAUSAL, {
      DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
          ...
          auto kernel_func = qk_int_sv_f16_attn_per_warp_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, DataType::kInt8, float, DTypeOut, ComputeUnit::kTensorCore, 
    ...
    DISPATCH_CAUSAL(is_causal, IS_CAUSAL, {
      DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
          ...
          auto kernel_func = qk_int_sv_f16_attn_per_warp_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, DataType::kInt8, half, DTypeOut, ComputeUnit::kTensorCore, 
                                                        mask_mode, RETURN_LSE, false>;

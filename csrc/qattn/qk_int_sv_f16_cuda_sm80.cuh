...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypePVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_mean=false>
                      ...
                      float sm_scale)
{
  ...
  static_assert(std::is_same<DTypePVAccum, float>::value || !use_inst_buffer, "use_inst_buffer only supports DTypePVAccum as float");
  static_assert(std::is_same<DTypePVAccum, float>::value || std::is_same<DTypePVAccum, half>::value, "DTypePVAccum must be float or half");
  ...
  static_assert(!fuse_v_mean || std::is_same<DTypePVAccum, half>::value, "fuse_v_mean only supports half");
  ...
  DTypePVAccum RO[num_tiles_q][num_tiles_v][8];
...
#pragma unroll
    ...
    {
      if constexpr (std::is_same<DTypePVAccum, float>::value)
      {
#pragma unroll
      ...
      else if constexpr (std::is_same<DTypePVAccum, half>::value)
      {
#pragma unroll
...
#pragma unroll
  ...
  if constexpr (std::is_same<DTypePVAccum, half>::value && std::is_same<DTypeOut, nv_bfloat16>::value)
  {
#pragma unroll
    for (uint32_t fq = 0; fq < num_tiles_q; fq++)
    {
...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, uint32_t qk_quant_gran, typename DTypePVAccum, bool use_inst_buffer, typename DTypeOut, bool is_causal, bool return_lse, bool fuse_v_mean>
  ...
  float sm_scale)
{
  ...
  auto kernel_func = qk_int_sv_f16_attn_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, head_dim, DataType::kInt8, static_cast<QuantGranularity>(qk_quant_gran), static_cast<QuantGranularity>(qk_quant_gran), DTypePVAccum, use_inst_buffer, DTypeOut, ComputeUnit::kTensorCore, mask_mode, return_lse, fuse_v_mean>;
